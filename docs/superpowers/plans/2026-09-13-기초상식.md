...
- **정답 위치를 고르게 섞는다.** 앱은 보기를 섞지 않고 저장된 순서대로 보여준다. 한 카테고리의 4지선다에서 네 위치가 각각 전체의 15~35%를 차지해야 하고, 벗어나면 검증 테스트가 실패한다(한국사 초안은 22/12/2/0이라 첫 번째만 눌러도 61%였다).
- **힌트는 정답 이름을 풀어 쓰지도 않는다.** 문자열이 그대로 들어가지 않아도 '성씨가 임금 왕 자', '경판이 8만 장' 같은 힌트는 사실상 정답이다. 다른 문제의 정답을 흘리는 힌트도 금지.
- **상급이 입문보다 쉬워지지 않게 한다.** 누구나 아는 날짜·사건은 상급에 두지 않는다.
